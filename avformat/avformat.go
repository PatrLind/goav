...
func AvUrlSplit(proto_size, authorization_size, hostname_size int, pp *int, path_size int, url string) (proto, authorization, hostname, path string) {
	Cproto := (*C.char)(C.malloc(C.sizeof_char * C.uint(proto_size)))
	defer C.free(unsafe.Pointer(Cproto))

	Cauthorization := (*C.char)(C.malloc(C.sizeof_char * C.uint(authorization_size)))
	defer C.free(unsafe.Pointer(Cauthorization))

	Chostname := (*C.char)(C.malloc(C.sizeof_char * C.uint(hostname_size)))
	defer C.free(unsafe.Pointer(Chostname))

	Cpath := (*C.char)(C.malloc(C.sizeof_char * C.uint(path_size)))
...
func AvGetFrameFilename(buf_size int, path string, number int) (int, string) {
	Cbuf := (*C.char)(C.malloc(C.sizeof_char * C.uint(buf_size)))
